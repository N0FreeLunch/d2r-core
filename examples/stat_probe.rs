...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Probing Stat Section in {} (0x{:04X}..0x{:04X})",
        path, payload_range.start, payload_range.end
    );
    ...
    for bits in [9, 10] {
        ...
        loop {
            ...
            if stat_id == terminator {
                println!(
                    "  [OK] Found Terminator (0x{:X}) at bit {} after {} stats.",
                    terminator, pos, count
                );
                break;
            }
            ...
            let val_bits = match stat_id {
                ...
                _ => {
                    println!(
                        "  [FAIL] Unknown Stat ID {} at bit {}. Desync likely.",
                        stat_id, pos
                    );
                    break;
                }
