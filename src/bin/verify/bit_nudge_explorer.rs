...
fn main() {
    let mut parser = ArgParser::new("BitNudgeExplorer").description(
        "Explores bit-level nudges for items in a D2R save file to find valid alignment",
    );
    ...
    parser.add_spec(
        ArgSpec::positional("item_index", "optional target item index to inspect").optional(),
    );
    ...
    for i in 0..count {
        ...
        if let Some(target) = target_idx {
            if current_idx != target {
                ...
                let mut reader =
                    IoBitReader::endian(Cursor::new(&bytes[jm_pos + 4..]), LittleEndian);
                let _ = reader.skip(bit_pos as u32);
                let mut cursor = BitCursor::new(reader);
                if let Ok(_) = Item::from_reader_with_context(
                    &mut cursor,
                    &huffman,
                    Some((&bytes, start_bit + bit_pos)),
                    is_alpha,
                    current_idx,
                    None,
                    None,
        ...
        for nudge in -4i64..=4i64 {
            ...
            if nudged_start >= (bytes.len() * 8) as u64 {
                continue;
            }
            ...
            match Item::from_reader_with_context(
                &mut cursor,
                &huffman,
                Some((&bytes, start_bit + nudged_start)),
                is_alpha,
                current_idx,
                None,
                None,
