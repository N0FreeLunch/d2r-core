...
fn main() {
    let mut parser = ArgParser::new("BitShiftScanner").description(
        "Scans for valid item headers by shifting the starting bit offset around the JM marker",
    );
    ...
    for shift in -16i64..=16i64 {
        ...
        if bit_start < 0 || bit_start >= (bytes.len() * 8) as i64 {
            continue;
        }
        ...
        match Item::from_reader_with_context(
            ...
            Ok(item) => {
                println!(
                    "  [Shift {:+3}] SUCCESS: '{}' (len={} bits, flags=0x{:08X}, compact={})",
                    shift,
                    item.code,
                    cursor.pos(),
                    item.flags,
