...
use std::process;
use d2r_core::verify::mutation::{mutate_absolute_bit_and_finalize, MutationMode};
use d2r_core::save::Save;

fn main() {
    let args: Vec<String> = env::args().collect();
    if args.len() < 3 {
        eprintln!("Usage: bsmt_mutate <save_file.d2s> <bit_offset> [--mode absolute|logical]");
        eprintln!("Example: bsmt_mutate player.d2s 2392");
        process::exit(1);
    }

    let input_path = Path::new(&args[1]);
    let bit_offset: usize = match args[2].parse() {
        Ok(val) => val,
        Err(_) => {
            eprintln!("Error: bit_offset must be a positive integer.");
            process::exit(1);
        }
    };
    ...
    if let Some(mode_idx) = args.iter().position(|r| r == "--mode") {
        if let Some(mode_val) = args.get(mode_idx + 1) {
            match mode_val.as_str() {
                ...
                "logical" => {
                    eprintln!("Error: Logical mode is not yet implemented.");
                    process::exit(1);
                }
                _ => {
    ...
    let mutated_bytes = match mode {
        MutationMode::Absolute => {
            println!("Mutation applied at bit offset: {}", bit_offset);
            match mutate_absolute_bit_and_finalize(&original_bytes, bit_offset) {
                Ok(bytes) => bytes,
                Err(e) => {
        ...
        MutationMode::Logical => unreachable!(),
    };
