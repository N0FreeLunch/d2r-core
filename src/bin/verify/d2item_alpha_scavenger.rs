...
use d2r_core::item::{BitRecorder, HuffmanTree, is_plausible_item_header};
use std::fs;
use std::io::Cursor;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        println!(
            "Usage: d2item_alpha_scavenger <save_path> [--start-bit <bit>] [--count <bits>] [--range <start> <end>]"
        );
        return Ok(());
    }
    ...
    if count > 0 {
        println!(
            "[AlphaScavenger] Dumping {} bits starting at {}...",
            count, start_bit
        );
    ...
    if range_end > 0 {
        ...
        for bit_pos in (start_aligned..=range_end).step_by(8) {
            ...
            let Ok(flags) = recorder.read_bits(32) else {
                continue;
            };
            let Ok(version) = recorder.read_bits(3) else {
                continue;
            };
            let version = version as u8;
            let Ok(mode) = recorder.read_bits(3) else {
                continue;
            };
            let mode = mode as u8;
            let Ok(loc) = recorder.read_bits(4) else {
                continue;
            };
            let loc = loc as u8;
            ...
            for offset in 32..=64 {
                ...
                if h_reader.skip((bit_pos + offset) as u32).is_err() {
                    continue;
                }
                let mut h_recorder = BitRecorder::new(&mut h_reader);

                if loc < 4 {
                    let _ = h_recorder.read_bits(8);
                }
                ...
                if valid && is_plausible_item_header(mode, loc, &code, flags, version, true) {
                    println!(
                        "  [FOUND] at bit {}: code='{}', flags={:#010x}, ver={}, mode={}, loc={}",
