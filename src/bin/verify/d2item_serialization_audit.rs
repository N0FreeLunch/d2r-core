...
fn main() {
    ...
    let target_idx = parsed.get("target").and_then(|s| s.parse::<usize>().ok());
...
fn generate_html_visual_report(
    report: &d2r_core::verify::symmetry::DiffReport,
    file_path: &str,
    out_path: &str,
) {
...
fn render_item_diff_html(
    html: &mut String,
    item: &d2r_core::verify::symmetry::ItemDiff,
    depth: usize,
) {
    let margin = depth * 24;
    html.push_str(&format!(
        "<div class=\"item-card\" style=\"margin-left: {}px;\">
    ...
    if let Some(offset) = item.first_mismatch_offset {
        html.push_str(&format!(
            "<div><strong>First Mismatch:</strong> Bit {}</div>",
            offset
        ));
    }
    ...
    if let (Some(orig), Some(target)) = (&item.orig_bits, &item.target_bits) {
        ...
        while i < o_chars.len() || j < t_chars.len() {
            ...
            if i < o_chars.len() && j < t_chars.len() && o_chars[i] == t_chars[j] {
                o_row.push_str(&format!(
                    "<div class=\"bit match{}\">{}</div>",
                    gap_class, o_chars[i]
                ));
                t_row.push_str(&format!(
                    "<div class=\"bit match{}\">{}</div>",
                    gap_class, t_chars[j]
                ));
                i += 1;
    ...
    if !item.is_match || item.fidelity_score < 1.0 {
        ...
        if let Some(offset) = item.first_mismatch_offset {
            ...
            if let (Some(orig), Some(target)) = (&item.orig_bits, &item.target_bits) {
                ...
                for i in start..end {
                    let color = if i == offset as usize {
                        ANSI_BOLD_RED
                    } else {
                        seg_color
                    };
                ...
                for i in start..end {
                    if i == offset as usize {
                        markers.push_str(&format!("{}^{}", ANSI_BOLD_RED, ANSI_RESET));
                    } else {
                        markers.push_str(" ");
                    }
                }
        ...
        if item.alpha_alignment_padding_len > 0 {
            om.println(&format!(
                "{:indent$}      [ALIGNMENT PAD] {} bits",
                "",
                item.alpha_alignment_padding_len,
                indent = indent.len()
            ));
        }
        if item.alpha_body_gap_len > 0 {
            om.println(&format!(
                "{:indent$}      [BODY GAP] {} bits",
                "",
                item.alpha_body_gap_len,
                indent = indent.len()
            ));
        }
        if item.fidelity_score < 1.0 {
            om.println(&format!(
                "{:indent$}      [FORENSIC RATIONALE]",
                "",
                indent = indent.len()
            ));
...
fn print_visual_diff(
        ...
        if i < o_chars.len() && j < t_chars.len() && o_chars[i] == t_chars[j] {
            ...
            for nudge in 1..49 {
                ...
                if j + nudge < t_chars.len() && o_chars[i] == t_chars[j + nudge] {
                    ...
                    let color = if is_gap_candidate && all_zeros {
                        ANSI_BLUE
                    } else {
                        ANSI_BOLD_RED
                    };
                    let marker = if is_gap_candidate && all_zeros {
                        "G"
                    } else {
                        "^"
                    };
                ...
                if i + nudge < o_chars.len() && o_chars[i + nudge] == t_chars[j] {
                    ...
                    let v_marker = if is_gap_candidate && all_zeros {
                        "G"
                    } else {
                        "v"
                    };
    ...
    let chunk_size = 80usize
        .saturating_sub(indent_level.saturating_mul(2))
        .max(24);
