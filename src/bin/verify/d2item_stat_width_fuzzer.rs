...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    parser
        .add_spec(ArgSpec::option("file", None, Some("file"), "Path to the save file").required());
    parser.add_spec(
        ArgSpec::option(
            "offset",
            None,
            Some("offset"),
            "Bit offset to start fuzzing from",
        )
        .required(),
    );
    ...
    if !use_json {
        println!(
            "Fuzzing bit-widths (1..32) at offset {} in {}",
            start_offset, file_path
        );
        println!("{:-<60}", "");
        println!(
            "{:>5} | {:>15} | {:>10} | {}",
            "Width", "Next JM Offset", "Alignment", "Status"
        );
        println!("{:-<60}", "");
    }
    ...
    for width in 1..=32 {
        ...
        for i in 0..1024 {
            ...
            if probe_pos + 16 > (bytes.len() * 8) as u64 {
                break;
            }
        ...
        if use_json {
            results.push(result);
        } else if alignment == 0 || width == 12 {
            // Always show 12 for runeword debugging
            println!(
                "{:>5} | {:>15?} | {:>10} | {}",
                result.width, result.next_jm, result.alignment_error, status
            );
        }
    }
