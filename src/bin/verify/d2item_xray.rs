...
fn main() -> Result<()> {
    ...
    parser.add_spec(ArgSpec::option(
        ...
        Some('c'),
        Some("compare"),
        "Path to the second fixture to compare against",
    ));
    parser.add_spec(ArgSpec::flag(
        "verbose",
        Some('v'),
        Some("verbose"),
        "Show detailed segment bitstreams",
    ));
    ...
    if let Some(c_items) = &compare_items {
        ...
        for exp in &exp_flattened {
            if let Some(act) = act_flattened
                .iter()
                .find(|a| a.path == exp.path && a.code == exp.code)
            {
                ...
                for j in 0..max_segments {
                    ...
                    match (e_seg, a_seg) {
                        (Some(e), Some(a)) => {
                            let status =
                                if e.label == a.label && e.start == a.start && e.end == a.end {
                                    // Check bits
                            ...
                            if status != " [PASS]" || verbose {
                                let e_bits_str: String = exp.bits[e.start as usize..e.end as usize]
                                    .iter()
                                    .map(|&b| if b { '1' } else { '0' })
                                    .collect();
                                let a_bits_str: String = act.bits[a.start as usize..a.end as usize]
                                    .iter()
                                    .map(|&b| if b { '1' } else { '0' })
                                    .collect();
        ...
        for (idx, item) in items.iter().enumerate() {
            ...
            let _reserialized = item
                .to_bytes(0, &huffman, is_alpha)
                .map_err(|e| anyhow::anyhow!("{}", e))?;
            ...
            for (c_idx, child) in item.socketed_items.iter().enumerate() {
                println!(
                    "    Socketed #{} [{}]: bits {}-{}",
                    c_idx, child.code, child.range.start, child.range.end
                );
            }
        }
    }
