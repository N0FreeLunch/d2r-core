...
fn handle_dump(args: &[String]) -> anyhow::Result<()> {
    ...
    let path = parsed
        .get("file")
        .context("Input file is required (use --file or -i)")?;
    let start_byte: u64 = parsed
        .get("start-byte")
        .map(|s| s.as_str())
        .unwrap_or("0")
        .parse()
    ...
    for i in 0..length {
        ...
        if byte_idx < bytes.len() {
            let bit = if (bytes[byte_idx] & (1 << bit_idx)) != 0 {
                '1'
            } else {
                '0'
            };
...
fn handle_diff(args: &[String]) -> anyhow::Result<()> {
    ...
    parser.add_spec(
        ArgSpec::option("file-a", Some('a'), Some("file-a"), "First file").value_count(1),
    );
    parser.add_spec(
        ArgSpec::option("file-b", Some('b'), Some("file-b"), "Second file").value_count(1),
    );
    parser.add_spec(
        ArgSpec::option(
            "bit-offset",
    ...
    parser.add_spec(
        ArgSpec::option("window", Some('w'), Some("window"), "Window size").value_count(1),
    );
    ...
    let path_a = parsed
        .get("file-a")
        .context("File A is required (use --file-a or -a)")?;
    let path_b = parsed
        .get("file-b")
        .context("File B is required (use --file-b or -b)")?;
    let target_offset: i64 = parsed
        .get("bit-offset")
        .context("Bit offset is required (use --bit-offset or -o)")?
        .parse()?;
    let window_size: i64 = parsed
        .get("window")
        .map(|s| s.as_str())
        .unwrap_or("64")
        .parse()
    ...
    for i in start_bit..end_bit {
        ...
        let bit_a = if byte_idx < bytes_a.len() {
            if (bytes_a[byte_idx] & (1 << bit_idx)) != 0 {
                "1"
            } else {
                "0"
            }
        ...
        let bit_b = if byte_idx < bytes_b.len() {
            if (bytes_b[byte_idx] & (1 << bit_idx)) != 0 {
                "1"
            } else {
                "0"
            }
