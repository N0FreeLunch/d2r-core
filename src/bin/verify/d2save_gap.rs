...
fn main() -> Result<()> {
    ...
    parser.add_spec(
        ArgSpec::option("min", None, Some("min"), "Minimum header bit length").with_default("56"),
    );
    parser.add_spec(
        ArgSpec::option("max", None, Some("max"), "Maximum header bit length").with_default("72"),
    );
    parser.add_spec(ArgSpec::option(
        "item-offset",
        None,
        Some("offset"),
        "Byte offset to item section",
    ));
    parser.add_spec(ArgSpec::flag(
        "probe-stats",
        None,
        Some("probe-stats"),
        "Enable stat bit-width probing",
    ));
    parser
        .add_spec(ArgSpec::option("id-min", None, Some("id-min"), "Min ID bits").with_default("7"));
    parser.add_spec(
        ArgSpec::option("id-max", None, Some("id-max"), "Max ID bits").with_default("11"),
    );
    parser.add_spec(
        ArgSpec::option("val-min", None, Some("val-min"), "Min Value bits").with_default("6"),
    );
    parser.add_spec(
        ArgSpec::option("val-max", None, Some("val-max"), "Max Value bits").with_default("14"),
    );
    parser.add_spec(ArgSpec::flag(
        "json",
        None,
    ...
    let min_len: u32 = parsed
        .get("min")
        .unwrap()
        .parse()
        .context("Invalid min length")?;
    let max_len: u32 = parsed
        .get("max")
        .unwrap()
        .parse()
        .context("Invalid max length")?;
    let item_offset_arg = parsed
        .get("item-offset")
        .map(|s| s.parse::<usize>())
        .transpose()
        .context("Invalid item-offset")?;
    ...
    for length in min_len..=max_len {
        ...
        let mut r_reader = BitReader::endian(
            Cursor::new(&bytes[(first_item_start / 8) as usize..]),
            LittleEndian,
        );
        ...
        let mut stat_probes = Vec::new();
        if parsed.is_set("probe-stats") && readability_score > 0.0 {
            let id_min: u32 = parsed.get("id-min").unwrap().parse().unwrap_or(7);
            let id_max: u32 = parsed.get("id-max").unwrap().parse().unwrap_or(11);
            let val_min: u32 = parsed.get("val-min").unwrap().parse().unwrap_or(6);
            let val_max: u32 = parsed.get("val-max").unwrap().parse().unwrap_or(14);

            let stat_start_bit = first_item_start + 32;

            for id_w in id_min..=id_max {
                for val_w in val_min..=val_max {
                    let mut reader = BitReader::endian(
                        Cursor::new(&bytes[(stat_start_bit / 8) as usize..]),
                        LittleEndian,
                    );
                    let skip = (stat_start_bit % 8) as u32;
                    if skip > 0 {
                        let _ = reader.skip(skip).unwrap_or(());
                    }
                    ...
                    loop {
                        ...
                        if id == terminator {
                            stat_probes.push(StatResult {
                                id_w,
                                val_w,
                                depth,
                                terminator_pos: current_pos,
                            });
                            break;
                        }
                        ...
                        if depth > 100 || current_pos > stat_start_bit + 5000 {
                            break;
                        }
                    }
                }
            }
            stat_probes.sort_by(|a, b| b.depth.cmp(&a.depth).then_with(|| a.id_w.cmp(&b.id_w)));
        }
    ...
    candidates.sort_by(|a, b| {
        b.readability_score
            .partial_cmp(&a.readability_score)
    ...
    if is_json {
        ...
        println!("File: {}", path);
        println!(
            "Item Section Heuristic Start: Byte {} (Bit {})",
            item_start_byte, item_start_bit
        );
        println!(
            "\n{:<10} {:<10} {:<10} {:<10} {:<15} {:<15}",
            "Header", "R-Score", "Score", "Markers", "Code", "Top Stat Probes"
        );
        for c in candidates.iter().take(20) {
            let stat_str = if c.stat_probes.is_empty() {
                "-".to_string()
            } else {
                let best = &c.stat_probes[0];
                format!("({},{}:d={})", best.id_w, best.val_w, best.depth)
            };
            println!(
                "{:<10} {:<10.1} {:<10.4} {:<10} {:<15} {:<15}",
                c.length,
                c.readability_score,
                c.score,
                c.markers.len(),
            ...
            if !c.stat_probes.is_empty() && c.readability_score > 500.0 {
                for sp in c.stat_probes.iter().take(3) {
                    println!(
                        "           -> ID:{}, Val:{}, Depth:{}",
                        sp.id_w, sp.val_w, sp.depth
                    );
                }
            }
        }
