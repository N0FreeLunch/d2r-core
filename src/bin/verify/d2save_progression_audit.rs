...
use d2r_core::domain::progression::axiom::{
    V105_QUEST_LEN, V105_QUEST_OFFSET, V105_WAYPOINT_LEN, V105_WAYPOINT_OFFSET,
};
use std::env;
use std::fs;
use std::process;
...
fn main() {
    ...
    parser.add_spec(ArgSpec::flag(
        ...
        None,
        Some("semantic"),
        "Print semantic status of quests and waypoints",
    ));
    parser.add_spec(ArgSpec::option(
        "mutate",
        None,
        Some("mutate"),
        "Apply mutations before parity check (comma-separated domain:target=state)",
    ));
    parser.add_spec(ArgSpec::option(
        "output",
        Some('o'),
        Some("output"),
        "Output mutated file to path",
    ));
        ...
        for part in target_str.split(',') {
            match parse_mutation_input(part) {
                Ok((domain, name, state)) => {
                    ...
                    match domain.as_str() {
                        "quest" => {
                            if let Some(q) = progression
                                .quests
                                .quests_mut()
                                .iter_mut()
                                .find(|q| q.name().eq_ignore_ascii_case(&normalized_name))
                            {
                                ...
                                eprintln!(
                                    "[ERROR] Quest '{}' not found in progression table.",
                                    name
                                );
                                process::exit(1);
                            }
                        ...
                        "waypoint" | "wp" => {
                            if let Some(w) = progression
                                .waypoints
                                .waypoints_mut()
                                .iter_mut()
                                .find(|w| w.name().eq_ignore_ascii_case(&normalized_name))
                            {
                                ...
                                eprintln!(
                                    "[ERROR] Waypoint '{}' not found in progression table.",
                                    name
                                );
                                process::exit(1);
                            }
                        ...
                        _ => {
                            eprintln!(
                                "[ERROR] Unknown domain '{}'. Use 'quest' or 'waypoint'.",
                                domain
                            );
                            process::exit(1);
                        }
    ...
    if semantic_mode {
        ...
        for quest in progression.quests.quests() {
            let status = if quest.is_completed() {
                "COMPLETED"
            } else {
                "pending"
            };
    ...
    if actual_bytes.len() >= quest_range.end && expected_bytes.len() >= quest_range.end {
        ...
        if expected_quest == actual_quest {
            ...
            for i in 0..V105_QUEST_LEN {
                if expected_quest[i] != actual_quest[i] {
                    println!(
                        "    First mismatch at offset 0x{:04X} (rel 0x{:X}): expected 0x{:02X}, got 0x{:02X}",
                        V105_QUEST_OFFSET + i,
                        i,
                        expected_quest[i],
                        actual_quest[i]
        ...
        println!(
            "  [FAIL] Quest Section: buffer size mismatch (len={}, required={})",
            actual_bytes.len(),
            quest_range.end
        );
        failures += 1;
    ...
    if actual_bytes.len() >= wp_range.end && expected_bytes.len() >= wp_range.end {
        ...
        println!(
            "  [FAIL] Waypoint Section: buffer size mismatch (len={}, required={})",
            actual_bytes.len(),
            wp_range.end
        );
        failures += 1;
    ...
    if failures > 0 {
        println!(
            "\nProgression Audit FAILED with {} section mismatch(es).",
            failures
        );
        process::exit(1);
    } else {
        ...
        if let Some(out_path) = output_path {
            ...
            match fs::write(out_path, &actual_bytes) {
                Ok(_) => println!(
                    "\n[EXPORT] Successfully wrote mutated save to: {}",
                    out_path
                ),
                Err(e) => {
...
fn parse_mutation_input(input: &str) -> Result<(String, String, String), String> {
    if let Some((domain_part, state)) = input.split_once('=') {
        ...
        Err(
            "Expected format 'domain:name=state' (e.g., quest:Sisters_to_the_Slaughter=completed)"
                .to_string(),
        )
    }
}
