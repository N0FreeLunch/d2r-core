    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if !self.diffs.is_empty() {
            ...
            for (idx, diff) in self.diffs.iter().enumerate().take(10) {
                writeln!(
                    f,
                    "  #{:02} [Bit {:4}] Expected: {}, Actual: {} | Context: {}",
                    idx,
                    diff.bit_offset,
                    if diff.expected { 1 } else { 0 },
                    if diff.actual { 1 } else { 0 },
                    diff.context
                )?;
            }
        } else {
...
fn main() -> io::Result<()> {
    ...
    if item_idx >= items.len() {
        return Err(io::Error::new(
            ...
            format!(
                "Item index {} not found. Total items: {}",
                item_idx,
                items.len()
            ),
        ));
    ...
    let expected_bits: Vec<BitInfo> = recorder
        .recorded_bits
        .iter()
        .map(|rb| {
...
fn generate_report(
    ...
    let status = if similarity >= 100.0 {
        "PERFECT"
