...
pub fn find_next_item_match(bytes: &[u8], pos: u64, huffman: &HuffmanTree, alpha: bool) -> Option<u64> {
    ...
    while probe < section_bits {
        ...
        if let Some((mode, location, _x, code, flags, version, is_compact, header_len, _nudge, _has_checksum)) = peek_item_header_at(bytes, probe, huffman, alpha, 0) {
...
pub fn is_plausible_item_header(
    ...
    if alpha_mode {
        if let Ok(s) = std::str::from_utf8(code) {
             let trimmed = s.trim();
             if trimmed == "xrs" || trimmed == "횧." { return mode <= 6 && location <= 5; }
        }
    }
    ...
    for (version, mode, loc, _x_val, base_header_len, has_checksum) in trial_configs {
        ...
        for gap in trial_possible_gaps {
            ...
            if alpha_mode {
                ...
                if success && (trimmed_ascii == "xrs" || trimmed_ascii == "횧." || is_v105_summary_code(&ascii_code)) {
                    t_code = ascii_code;
                } else {
                    t_cursor.rollback(huffman_pos);
                }
            ...
            if trimmed.len() >= 2 {
                ...
                let is_known = reg.forced_compact_codes.as_ref().map(|codes| codes.iter().any(|c| c == trimmed)).unwrap_or(false)
                    ...
                    || trimmed == "acww" || trimmed == "bcww" || trimmed == "xrs" || trimmed == "횧.";

                if is_known { confidence += 400; }
                if alpha_mode && (trimmed == "hp1" || trimmed == "xrs") { 
                    confidence += 500; 
                    if trimmed == "xrs" && (flags & (1 << 26)) != 0 { confidence += 1000; }
                }
    ...
    pub fn from_reader_with_context<R: BitRead>(
        ...
        let detected_runeword = header.is_runeword
            || (header.save_is_alpha
                && matches!(body.code.trim(), "xrs" | "c8xr" | "횧."));
        ...
        let axiom = StatsAxiom::new(item.header.version, item.header.quality.unwrap_or(ItemQuality::Normal), alpha_mode)
            ...
            .with_code(&item.code);
        let nudge_comb = NudgeCombinator;
        let padding = nudge_comb.apply_alignment_padding(cursor, start_bit, &item.code, item.header.flags, &axiom)?;
...
pub fn scan_socket_children(
    ...
    while current_pos < max_pos && current_pos < section_bits {
        if let Some((mode, location, _x, code, flags, version, _is_compact, _header_bits, _nudge, _has_checksum)) = peek_item_header_at(bytes, current_pos, huffman, alpha, 0) {        
...
pub fn read_player_name<R: BitRead>(cursor: &mut BitCursor<R>, _alpha_v5: bool) -> ParsingResult<String> {
