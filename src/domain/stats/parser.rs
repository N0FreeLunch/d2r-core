...
use crate::domain::stats::{
    lookup_alpha_map_by_raw, ItemProperty,
};
...
where 
    F: FnMut(&mut BitCursor<R>, &str, &'a [u8], u64, &HuffmanTree) -> ParsingResult<bool>
{
    recorder.push_context("Property List");
    let mut props = Vec::new();
    let mut recovery_fn = recovery_fn;
    ...
    let stat_id = match recorder.read_bits::<u32>(id_bits) {
        ...
        Err(err) => {
            if let Some((section_bytes, item_start_bit)) = section_recovery {
                if recovery_fn(recorder, code, section_bytes, item_start_bit, huffman)? {
                    recorder.end_segment();
