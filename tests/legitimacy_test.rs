...
use d2r_core::item::{Item, ItemQuality, ItemProperty};
use d2r_core::engine::validation::{check_socket_legitimacy, check_alvl_legitimacy, check_staffmod_legitimacy};
...
fn test_socket_legitimacy_violation() {
    ...
    assert!(!warnings.is_empty(), "Should warn about socket count exceeding max for ilvl 1");
    // "Socket count 6 exceeds max 3 for ilvl 1"
    assert!(warnings[0].contains("exceeds max 3"), "Warning missing 'exceeds max 3': {}", warnings[0]);
}
...
fn test_socket_legitimacy_valid() {
    ...
    let warnings = check_socket_legitimacy(&item);
    assert!(warnings.is_empty(), "Should NOT warn for valid socket count: {:?}", warnings);
}
...
fn test_alvl_legitimacy_violation() {
    ...
    assert!(!warnings.is_empty(), "Should warn about Cruel prefix on ilvl 1 Long Sword");
    assert!(warnings[0].contains("Cruel"), "Warning missing 'Cruel': {}", warnings[0]);
    assert!(warnings[0].contains("item aLvl 10"), "Expected aLvl 10 mismatch: {}", warnings[0]);
}
...
fn test_alvl_legitimacy_valid() {
    ...
    let warnings = check_alvl_legitimacy(&item);
    assert!(warnings.is_empty(), "Should NOT warn about Cruel on ilvl 99 item: {:?}", warnings);
}
...
fn test_staffmod_legitimacy_violation() {
    ...
    assert!(!warnings.is_empty(), "Should warn about high tier Automod on ilvl 1 shield");
    assert!(warnings[0].contains("requires ilvl 39"), "Warning mismatch: {}", warnings[0]);
}
...
fn test_staffmod_legitimacy_valid() {
    ...
    let warnings = check_staffmod_legitimacy(&item);
    assert!(warnings.is_empty(), "Should NOT warn for valid Automod tier: {:?}", warnings);
}
...
fn test_alvl_legitimacy_with_magic_lvl() {
    ...
    let warnings = check_alvl_legitimacy(&item);
    assert!(warnings.is_empty(), "Knight's (lvl 25) should be valid on ilvl 1 Circlet (aLvl 27): {:?}", warnings);

    // Prefix "Meteoric" (id 33), level 27. Should be VALID (27 <= 27).
    item.magic_prefix = Some(33);
    let warnings = check_alvl_legitimacy(&item);
    assert!(warnings.is_empty(), "Meteoric (lvl 27) should be valid on ilvl 1 Circlet (aLvl 27): {:?}", warnings);

    // Prefix "Platinum" (id 32), level 22. Should be VALID.
    item.magic_prefix = Some(32);
    let warnings = check_alvl_legitimacy(&item);
    assert!(warnings.is_empty(), "Platinum (lvl 22) should be valid on ilvl 1 Circlet (aLvl 27)");
}
...
fn test_affix_group_legitimacy_violation() {
    ...
    assert!(!warnings.is_empty(), "Should warn about duplicate affix group 101");
    assert!(warnings[0].contains("Duplicate affix group found: 101"), "Warning mismatch: {}", warnings[0]);
}
...
fn test_ethereal_legitimacy_violation() {
    ...
    assert!(!warnings.is_empty(), "Should warn about low ethereal durability");
    assert!(warnings[0].contains("lower than expected durability"), "Warning mismatch: {}", warnings[0]);
    assert!(warnings[0].contains("expected ~23"), "Warning missing expected value: {}", warnings[0]);
}
...
fn test_ethereal_legitimacy_valid() {
    ...
    let warnings = d2r_core::engine::validation::check_ethereal_legitimacy(&item);
    assert!(warnings.is_empty(), "Should NOT warn for correct ethereal durability: {:?}", warnings);
}
...
fn test_illegal_ethereal_bow() {
    ...
    assert!(warnings[0].contains("Bows and crossbows cannot be ethereal"), "Warning missing 'cannot be ethereal': {}", warnings[0]);
}
