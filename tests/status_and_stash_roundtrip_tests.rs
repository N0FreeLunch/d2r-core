...
fn test_variable_length_rebuild() -> io::Result<()> {
    let bytes = fs::read(repo_path("tests/fixtures/savegames/original/amazon_empty.d2s"))?;
    let huffman = HuffmanTree::new();
    let map = map_core_sections(&bytes)?;
    ...
    assert!(rebuilt.len() >= original_len, "Rebuilt save should be at least as large as original");
    
    // Check that we can parse it back
    let new_map = map_core_sections(&rebuilt)?;
    let new_attrs = AttributeSection::parse(&rebuilt, &new_map)?;
    
    let found = new_attrs.entries.iter().any(|e| e.stat_id == 16 && e.raw_value == 42);
    ...
    assert_eq!(file_size_in_header, rebuilt.len() as u32, "Header file size should be updated");
